...
from models import MLP_BIC, MLP_color, cGAN 
from utils import evaluate_gan_minmax_inverse
import wandb
...
def evaluate(model, val_loader, test_loader, optimizer_G, optimizer_D,forward_color_model, forward_BIC_model, criterion, prior, test=False):
    ...
    with torch.no_grad():
        ...
        cie_raw, param_raw, cie_pred, param_pred = evaluate_gan_minmax_inverse(model, forward_color_model,forward_BIC_model, val_loader.dataset, show=0)
...
def main(configs):
    ...
    # start training 
    path = configs.save_path + "/" + configs.name + ".pth"
    path_temp = configs.save_path + "/" + configs.name + "_temp.pth"     
    ...
    for e in range(epochs):
        ...
        loss_val, rmse_val = evaluate(model, val_loader, test_loader, optimizer_G, optimizer_D, forward_color_model,forward_BIC_model,  criterion, configs.prior)
...
if __name__  == '__main__':
    ...
    parser.add_argument('--output_dim', type=int, default=6, help='Output size of generator')
    ...
    parser.add_argument('--name', type=str, default="cGAN_01", help='Run multiple times and save ')
    parser.add_argument('--notes', type=str, default="cGAN第1次尝试", help='Run multiple times and save ')
