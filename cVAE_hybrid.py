...
from models import cVAE_hybrid,MLP_BIC,MLP_color,cVAE
...
def train(model, train_loader, optimizer, criterion, configs):
    ...
    for x, y in train_loader:
        ...
        recon_loss = criterion(recon_x, x)  # 重构误差，衡量模型输出与输入数据之间的差异
        # replace_loss = criterion(x_hat, x)  # 替换误差，衡量模型生成的数据与输入数据之间的差异
        KLD = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())
        pred_loss = criterion(y_pred, y)

        if configs.add_forward:
            # when added forward model as a critic on loss
            loss = recon_loss  + KLD + configs.weight_forward * pred_loss
...
def main(configs):
    ...
    vae_model = cVAE(config.input_dim, config.latent_dim).to(DEVICE)
    ...
    optimizer = torch.optim.Adam(model.vae_model.parameters(), lr=config.lr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dim', type=int, default=4, help='Output dim of y')
    ...
    parser.add_argument('--color_model', type=str, default="forward_03", help='Run multiple times and save ')
    parser.add_argument('--BIC_model', type=str, default="forwardBIC_02", help='Run multiple times and save ')
    parser.add_argument('--name', type=str, default="VAE_03", help='Run multiple times and save ')
    parser.add_argument('--notes', type=str, default="VAE第3次尝试", help='Run multiple times and save ')
