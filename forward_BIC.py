...
from datasets import get_BIC_dataloaders
from torch.optim.lr_scheduler import StepLR
from models import MLP_BIC
import wandb
...
def main(configs):
    ...
    train_loader, val_loader, test_loader = get_BIC_dataloaders('forward_model', config.batch_size)

    model = MLP_BIC(config.input_dim, config.output_dim).to(DEVICE)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dim', type=int, default=1, help='Output dimension of y')
    ...
    parser.add_argument('--name', type=str, default="forwardBIC_02", help='Run multiple times and save ')
    parser.add_argument('--notes', type=str, default="前向BIC第2次尝试", help='Run multiple times and save ')
